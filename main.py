...
def analyze_reviews(app_id, keywords_dict, lang='id', country='id', sort=Sort.NEWEST, filter_score_with=""):
    def scrape_reviews_batched(app_id, lang='id', country='id', sort=Sort.NEWEST, filter_score_with=""):
        ...
        for _ in range(9):  # Scrape 9 batches (adjust as needed)
            result, continuation_token = reviews(app_id, lang=lang, country=country, sort=sort, count=100, filter_score_with=filter_score_with)
    ...
    vectorizer = CountVectorizer().fit_transform(keywords_list + reviews)
    ...
    for i, keyword in enumerate(keywords_list):
        relevant_reviews = [(reviews[j], similarity) for j, similarity in enumerate(similarities[i]) if similarity > threshold]
        if relevant_reviews:
            keyword_to_reviews[keyword] = relevant_reviews
    
    # Display the categorized reviews with sentiment analysis and summarize the sentiment
    for keyword, review_list in keyword_to_reviews.items():
        ...
        for review, score in review_list:
            ...
            print(f"Review: {review}, Score: {score:.4f}, Sentiment Rating: {sentiment_rating}")
        
        if sentiment_ratings:
            avg_sentiment = sum(sentiment_ratings) / len(sentiment_ratings)
            print(f"Average Sentiment Rating for '{keyword}': {avg_sentiment:.2f}")
    ...
    if overall_sentiment_ratings:
        ...
        print(f"\nOverall Average Sentiment Rating: {overall_avg_sentiment:.2f}")
...
analyze_reviews('id.or.muhammadiyah.quran', keywords_dict)
